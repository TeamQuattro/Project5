...
var freeTimeTest = function freeTimeTest() {
	$(".freeTimeInput").on("submit", function (e) {
		e.preventDefault();
...
booksMovies.getMovieInfo = function (userMovieChoice) {
	$.ajax({
...
booksMovies.displayMovieInfo = function (movieResults) {
	for (var i = 0; i < 4; i++) {
		// 	movie backdrop path
		var movBg = "https://image.tmdb.org/t/p/w1400_and_h450_bestv2" + movieResults[i].backdrop_path;
		// movie poster
		var movImage = $("<img>").attr("src", "https://image.tmdb.org/t/p/w500" + movieResults[i].poster_path).attr("alt", movieResults[i].title + " poster image");
		//movie title <h2> and adding to page
		var movTitle = $("<h2>").text(movieResults[i].title);
		//movie year
		var movYear = movieResults[i].release_date.split("-");
		movYear = $("<h3>").text(movYear[0]);
		//select movie button
		var movSelect = $("<button>").addClass("movieSelect").text("select");
		// container for movies 
		var movContainer = $("<div>").addClass("movie wow fadeInLeft").attr("data-movieBg", movBg).append(movImage, movTitle, movYear, movSelect);
		// .attr('data', 'wideimage')
...
booksMovies.getData = function (query) {
	...
	$.ajax({
		...
		// <-- Then should be based on selected book from list of arrays
		console.log(res);
		...
		var previewLink = res.items[0].volumeInfo.previewLink;
		var bookLink = res.items[0].volumeInfo.infoLink;
